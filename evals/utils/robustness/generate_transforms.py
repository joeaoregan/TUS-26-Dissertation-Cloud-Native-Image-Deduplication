#!/usr/bin/env python3
"""
Generate robustness transform variants from base images.

Input:
  data/robustness/base/

Output:
  data/robustness/<transform_bucket>/
  data/robustness/transform_manifest.csv
"""
...
def stem_safe_name(source: Path) -> str:
    # Flatten nested paths into traceable token if needed
    rel = source.relative_to(BASE_DIR)
    token = "__".join(rel.with_suffix("").parts)
    return token
...
def generate() -> list[OutputRecord]:
    ...
    crop_factors = [0.90, 0.75]  # retained central area
    rotate_degrees = [2, -2]
    ...
    for src in iter_base_images(BASE_DIR):
        ...
        with Image.open(src) as im_raw:
            ...
            for channel, delta in channel_shifts:
                ...
                r, g, b = im.split()
                bands = {"r": r, "g": g, "b": b}
                shifted = bands[channel].point(
                    lambda x, d=delta: max(0, min(255, x + d))
                )
                bands[channel] = shifted
