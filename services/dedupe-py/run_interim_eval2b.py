...
#
# Outputs:
# - Detection-quality summary for report Table 4
# - Computational-cost summary for report Table 5
# - Benchmark JSON reports
# - Stage 3 prediction CSV files
# - Benchmark, export and evaluation logs
# ============================================================
...
DETECTION_SUMMARY_CSV = METRICS_DIR / "table_eval2_detection_summary.csv"

COST_SUMMARY_CSV = METRICS_DIR / "table_eval2_cost_summary.csv"


# ============================================================
# Staging locations required by current path-safety rules
#
# export_predictions accepts benchmark JSON under benchmarks/.
#
# export_predictions and evaluate_predictions accept CSV files
# under data/reviews/.
# ============================================================
...
def build_environment() -> dict:
    """
    Configure PYTHONPATH so subprocesses can import the
    benchmarks package.
    """
    env = os.environ.copy()
...
def parse_eval_metrics(text: str) -> dict:
    """Extract detection-quality metrics from evaluator output."""
...
def parse_benchmark_cost(
    json_path: Path,
) -> dict:
    """
    Extract runtime, memory and pair counts for report Table 5.
    """
    ...
    return {
        ...
        "TotalTimeMsMean": number(
            get_value(
                "total_pipeline_time_ms",
                "mean",
            )
        ),
        "TotalTimeMsStd": number(
            get_value(
                "total_pipeline_time_ms",
                "std_dev",
            )
...
def validate_inputs() -> None:
    """Ensure required dataset and label inputs exist."""
...
def prepare_directories() -> None:
    """Create final and staging output directories."""
    directories = [
        METRICS_DIR,
        LOGS_DIR,
        PREDICTIONS_DIR,
...
def prepare_reference_labels() -> None:
    """Stage labels under data/reviews for the evaluator."""
    shutil.copy2(
        REFERENCE_LABELS_SOURCE,
        REFERENCE_LABELS_STAGED,
    )
...
def format_table_cell(
    ...
    """Format a table cell without ANSI colour codes."""
    text = str(value)
...
def print_table_header(
    ...
    """Print a coloured table title and column headings."""
    header = (
        "| "
        + " | ".join(
            format_table_cell(
...
def print_table_footer(
    ...
    """Print a table's closing border."""
...
def write_detection_summary(
    ...
    """Write the CSV used to populate report Table 4."""
    fieldnames = [
        "ConfigID",
        "RunTag",
        "pHashThreshold",
    ...
    with DETECTION_SUMMARY_CSV.open(
        "w",
        newline="",
        encoding="utf-8",
    ) as csv_file:
...
def write_cost_summary(
    ...
    """Write the CSV used to populate report Table 5."""
    fieldnames = [
        "ConfigID",
        "RunTag",
        "pHashThreshold",
    ...
    # Explicitly select only the fields required by the CSV.
    # This prevents console-only fields such as Stage1Time
    # from being passed to csv.DictWriter.
    csv_rows = [{field: row.get(field, "") for field in fieldnames} for row in rows]

    with COST_SUMMARY_CSV.open(
        "w",
        newline="",
        encoding="utf-8",
    ) as csv_file:
...
def main() -> None:
    ...
    # Table 4 headings are printed before C1 starts.
    # Each row appears immediately after its configuration finishes.
    print_table_header(
        "Table 4 - Detection Performance Across Threshold Configurations",
        DETECTION_TABLE_COLUMNS,
    )
    ...
    # ========================================================
    # Table 5 console output
    # ========================================================

    print_table_header(
        "Table 5 - Computational Cost by Configuration",
        COST_TABLE_COLUMNS,
    )
